...
async function firestoreGet(token, projectId, docPath) {
...
async function firestorePatch(token, projectId, docPath, fields) {
  return new Promise((res) => {
    const body = JSON.stringify({ fields });
    const fieldMask = Object.keys(fields).join(',');
    const req = https.request({
      hostname: 'firestore.googleapis.com',
      path: `/v1/projects/${projectId}/databases/(default)/documents/${docPath}?updateMask.fieldPaths=${encodeURIComponent(fieldMask)}`,
      method: 'PATCH',
      headers: {
...
async function run() {
  ...
  // ── Get Firestore token once for all operations ───────────────
  let fsToken = null, fsProjectId = null;
  if (CFG.firebaseSA) {
    try {
      const sa = JSON.parse(CFG.firebaseSA);
  ...
  if (fsToken) {
    try {
      const doc = await firestoreGet(fsToken, fsProjectId, 'appdata/sentLog');
      if (doc && doc.fields && doc.fields.value && !doc.error) {
        sentLog = JSON.parse(doc.fields.value.stringValue || '{}');
        console.log(`  ☁ Loaded sent-log from Firestore (${Object.keys(sentLog).length} entries)`);
      }
    } catch(e) { console.warn('  ⚠ Could not load sent-log from Firestore:', e.message); }
  }
  // Merge with local file (in case Firestore was empty but local has entries)
  try {
    if (fs.existsSync(SENT_LOG)) {
      const local = JSON.parse(fs.readFileSync(SENT_LOG, 'utf8'));
      sentLog = Object.assign({}, local, sentLog); // Firestore wins on conflict
    }
  } catch(e) {}

  // ── Load notifLog from Firestore ──────────────────────────────
  let notifLog = [];
  if (fsToken) {
    try {
      const doc = await firestoreGet(fsToken, fsProjectId, 'appdata/notifLog');
      if (doc && doc.fields && doc.fields.value && !doc.error) {
        notifLog = JSON.parse(doc.fields.value.stringValue || '[]');
      }
    } catch(e) { console.warn('  ⚠ Could not load notifLog from Firestore:', e.message); }
  }
...
async function saveSentLog(sentLog, token, projectId) {
  ...
  for (const k in sentLog) {
    if (new Date(sentLog[k]) < cutoff) delete sentLog[k];
  }
  // Save to local file
  try {
    fs.mkdirSync(path.dirname(SENT_LOG), { recursive: true });
    fs.writeFileSync(SENT_LOG, JSON.stringify(sentLog, null, 2), 'utf8');
  } catch(e) { console.warn('  ⚠ Could not write local sent-log:', e.message); }
  // Save to Firestore
  if (token && projectId) {
    try {
      await firestorePatch(token, projectId, 'appdata/sentLog', {
        value: { stringValue: JSON.stringify(sentLog) }
      });
      console.log(`  ☁ Sent-log saved to Firestore (${Object.keys(sentLog).length} entries)`);
    } catch(e) { console.warn('  ⚠ Could not save sent-log to Firestore:', e.message); }
  }
}

async function saveNotifLog(notifLog, token, projectId) {
  if (!token || !projectId) return;
  try {
    await firestorePatch(token, projectId, 'appdata/notifLog', {
      value: { stringValue: JSON.stringify(notifLog) }
    });
    console.log(`  ☁ Notification log saved to Firestore`);
  } catch(e) { console.warn('  ⚠ Could not save notifLog to Firestore:', e.message); }
}
