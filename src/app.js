/* ═══════════════════════════════════════════════════════════════
   ...
   Full rebuild: automated + manual notifications, Entries field,
   Warsaw timezone, finish/ext status, auto-refresh every 60s
   ═══════════════════════════════════════════════════════════════ */
'use strict';
...
/* ═══════════════════════════════════════════════════════════════
   WARSAW TIME  — never use toISOString() for date comparisons
   ═══════════════════════════════════════════════════════════════ */
...
function parseDateSafe(rawDate) {
  ...
  // "2026-05-21 00:00:00" or "2026-05-21T00:00:00" — just slice first 10 chars
  if (/^\d{4}-\d{2}-\d{2}[T \d]/.test(raw)) return raw.substring(0, 10);
  // "DD/MM/YYYY"
  ...
  if (/^\d{2}\/\d{2}\/\d{2}$/.test(raw)) {
    const [d,m,y] = raw.split('/'); return `20${y}-${m}-${d}`;
  }
  // "21 May 26" or "21 May 2026"
  if (/^\d{1,2}\s+[A-Za-z]{3}\s+\d{2,4}$/.test(raw)) {
    const months = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',
                    jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};
    const parts = raw.split(/\s+/);
    const mon   = months[parts[1].toLowerCase().substring(0,3)] || '01';
    const yr    = parts[2].length===2 ? '20'+parts[2] : parts[2];
    return `${yr}-${mon}-${parts[0].padStart(2,'0')}`;
  }
  // Excel serial number "45802"
  if (/^\d{5}$/.test(raw)) {
...
function examStatus(exam) {
  ...
  if (exam.date > today) {
    const daysAway = Math.round((new Date(exam.date) - new Date(today)) / 86400000);
    return { cls:'status-future', label: daysAway===1 ? 'Tomorrow' : `In ${daysAway}d` };
  }
...
function initApp() {
  ...
  renderLog();
  updateStats();
  startAutoEngine();   // ← start the automated notification engine
}
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Runs every 60s inside the browser tab when admin is logged in.
   GitHub Actions (notify.js) runs independently every 15 min
   even when no admin is logged in — covering both cases.
   ═══════════════════════════════════════════════════════════════ */
...
async function runAutoCheck(verbose) {
  const s = getSettings();
  if (!s.autoNotifyEnabled) return;
  if (!s.ejsPublicKey || !s.ejsServiceId || !s.ejsTemplateId) return;
  ...
  for (const exam of exams) {
    ...
    const minsUntil = startMins - nowMins;
    // Window: notifyMinutes ± 2 min (60s loop gives tight window)
    if (minsUntil < notifyMinutes - 2 || minsUntil > notifyMinutes + 2) continue;
  ...
  if (verbose || toSend.length) {
    showToast(`Auto-sent ${toSend.length} notification(s) ✓`, 'success');
  }
}

/* Manual full check from settings panel */
async function runNotificationCheck() {
  ...
  showResult(resultEl,'⏳ Scanning all exams…','info');
  const today   = warsawTodayISO();
  const nowMins = warsawNowMinutes();
  const { notifyMinutes } = s;
  const toSend  = [];
  ...
  if (!toSend.length) {
    showResult(resultEl,
      `✅ No notifications due right now.\n` +
      `${exams.length} exams loaded · ${today} · now: ${minsToTime(nowMins)} Warsaw\n` +
      `System fires automatically ${notifyMinutes} min before each exam.`,'ok');
    return;
  }
  ...
  S.set('exams', exams);
  renderDashboard(); renderTimetable(); renderLog(); updateStats();
  showResult(resultEl,`✅ Sent ${sent} · Failed ${failed} of ${toSend.length}`, failed>0?'err':'ok');
}
...
/* ═══════════════════════════════════════════════════════════════
   CSV PARSER — robust, handles quoted fields with commas inside
   ═══════════════════════════════════════════════════════════════ */
function csvRowToFields(line) {
  // Proper RFC-4180 CSV parser — handles "field, with, commas" and ""escaped quotes""
  const fields = [];
  let inQ = false, cell = '';
  for (let i = 0; i < line.length; i++) {
    const ch = line[i];
    if (ch === '"') {
      if (inQ && line[i+1] === '"') { cell += '"'; i++; }  // "" → literal "
      else inQ = !inQ;
    } else if (ch === ',' && !inQ) {
...
/* ═══════════════════════════════════════════════════════════════
   PARSE TIMETABLE CSV — with full diagnostic logging
   ═══════════════════════════════════════════════════════════════ */
function parseTimetableCSV(csv) {
  const diag = [];   // diagnostic messages shown to admin
  const rows = csvToRows(csv);
  diag.push(`📄 CSV received: ${rows.length} lines total`);
  ...
  if (hi === -1) {
    showBanner('❌ Header row not found — cannot see "Start time" AND "Invigilator" columns. Check the correct sheet tab is published.','err');
    return 0;
  }
  diag.push(`✅ Header row found at line ${hi}`);

  // ── 2. Map column indices ───────────────────────────────────
  // CRITICAL: do NOT use .trim() on headers before searching —
  // col 8 is "Exam Date " (trailing space), col 5 is "Exam Date" (no space).
  // We use the RAW header to distinguish them, then find by exact column position.
  const H_raw  = rows[hi];   // raw headers — preserve trailing spaces
  const H      = H_raw.map(c => (c||'').toLowerCase().trim()); // trimmed for most lookups
  const H_notrimmed = H_raw.map(c => (c||'').toLowerCase());   // untrimmed for duplicate detection

  // col() finds first match using trimmed headers
  const col = kw => H.findIndex(h => h.includes(kw.toLowerCase()));

  // For "Exam Date" — col 5 is the range (e.g. "01 Mar - 30 Apr"), col 8 is the actual date.
  // We detect col 8 by finding the SECOND occurrence of "exam date"
  function colNthExamDate(n) {
    let count = 0;
    for (let i = 0; i < H.length; i++) {
      if (H[i].includes('exam date')) { count++; if (count === n) return i; }
    }
    return -1;
  }

  const C = {
    date:      colNthExamDate(2),   // 2nd "Exam Date" = col 8 (the actual datetime)
    room:      col('room'),
    session:   col('session'),
  ...
  // entries: prefer the FIRST "entries" occurrence (col 6), skip the duplicate at end
  if (C.entries === -1) {
    C.entries = H.findIndex((h,i) => h === 'entries');
  }

  diag.push(`🗂 Column mapping: date=${C.date} start=${C.start} finish=${C.finish} invig=${C.invig} backup=${C.backup} entries=${C.entries} syllabus=${C.syllabus}`);

  // Warn about any missing critical columns
  const missing = [];
  if (C.date     < 0) missing.push('Exam Date (2nd occurrence)');
  if (C.start    < 0) missing.push('Start time');
  if (C.syllabus < 0) missing.push('Syllabus');
  if (C.invig    < 0) missing.push('Exam invigilator');
  if (missing.length) {
    diag.push(`⚠️ Missing columns: ${missing.join(', ')}`);
  }

  // ── 3. Parse data rows ──────────────────────────────────────
  const existing   = new Map(exams.map(e => [e.id, e]));
  const parsed     = [];
  let skippedBlank = 0, skippedDate = 0, skippedTime = 0, skippedSyllabus = 0;

  for (let i = hi + 1; i < rows.length; i++) {
    const r = rows[i];
    if (!r || r.every(c => !c || !c.trim())) { skippedBlank++; continue; }

    const rawDate    = C.date     >= 0 ? (r[C.date]     || '').toString().trim() : '';
    const rawStart   = C.start    >= 0 ? (r[C.start]    || '').toString().trim() : '';
    const syllabus   = C.syllabus >= 0 ? (r[C.syllabus] || '').toString().trim() : '';

    if (!syllabus) { skippedSyllabus++; continue; }

    if (!rawDate || rawDate === 'NaN' || rawDate === '') {
      skippedDate++; continue;
    }

    const dateStr = parseDateSafe(rawDate);
    if (!dateStr || !/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
      skippedDate++;
      if (parsed.length === 0 && skippedDate <= 3) {
        diag.push(`⚠️ Row ${i}: date "${rawDate}" could not be parsed → skipped`);
      }
      continue;
    }

    if (!rawStart || rawStart === 'NaN') { skippedTime++; continue; }
    const startMins = parseTimeToMins(rawStart);
    if (startMins == null) {
      skippedTime++;
      if (parsed.length === 0 && skippedTime <= 3) {
        diag.push(`⚠️ Row ${i}: start time "${rawStart}" could not be parsed → skipped`);
      }
      continue;
    }
  ...
  // ── 4. Summary ──────────────────────────────────────────────
  diag.push(`📊 Results: ${parsed.length} imported · ${skippedBlank} blank · ${skippedSyllabus} no-subject · ${skippedDate} bad-date · ${skippedTime} bad-time`);

  if (parsed.length === 0) {
    showBanner(
      `❌ 0 exams imported from ${rows.length} lines.\n` +
      diag.map(d => `  ${d}`).join('\n'),
      'err'
    );
    console.warn('[Exam Notifier] Sync diagnostics:', diag);
    return 0;
  }
  ...
  showBanner(
    `✅ ${parsed.length} exams imported · ${todayCount} today · ` +
    `skipped: ${skippedBlank} blank, ${skippedSyllabus} no-subject, ${skippedDate} bad-date, ${skippedTime} bad-time`,
    'ok'
  );

  // Log diagnostics to console for debugging
  console.info('[Exam Notifier] Sync complete:', diag.join(' | '));
...
function renderDashboard() {
  ...
  const limStr = limit.toLocaleDateString('sv-SE',{timeZone:'Europe/Warsaw'});
    ...
    <div class="exam-card">
      ...
      <div class="exam-info">
        ...
        <div class="exam-meta">
          ...
          ${exam.entries ?`<span class="tag tag-entries">👥 ${esc(exam.entries)} student${exam.entries!=='1'?'s':''}</span>`:''}
...
function renderTimetable() {
  ...
  <table class="data-table">
    <thead><tr>
      ...
      <th>Room</th><th>Students</th><th>Main Invigilator</th><th>Backup</th><th>Status</th><th></th>
    </tr></thead>
    <tbody>${list.map(e=>{
      const st=examStatus(e);
      const inv=resolveInvigilator(e.invigRaw);
      const bak=resolveInvigilator(e.backupRaw);
      return `<tr>
        ...
        <td>${inv?`<span class="tag tag-invig">${esc(inv.name)}</span>`:'<span style="color:var(--text-3)">—</span>'}</td>
        <td>${bak?`<span class="tag tag-backup">${esc(bak.name)}</span>`:'<span style="color:var(--text-3)">—</span>'}</td>
...
async function sendOneEmail(exam, person, role) {
  ...
  const params={
    ...
    num_entries:    exam.entries||'N/A',   // ← student count
    role,
...
async function manualNotify(examId) {
  ...
  for(const[raw,role,type]of[
    ...
    const person=resolveInvigilator(raw);if(!person)continue;
    try{
      ...
      logEntry(exam,person.name,role,resolveEmail(person),false,e.text||e.message);failed++;
    }
    await sleep(400);
  }
  S.set('exams',exams);
...
async function sendTestEmail() {
  ...
  try{
    ...
    showResult(resultEl,'✅ Test email sent! Check the inbox in your EmailJS template.','ok');
...
function loadSettingsUI(){
  ...
  if(tog) tog.checked=s.autoNotifyEnabled!==false;
}
...
function saveNotifSettings(){
  ...
  renderInvigilators();showToast('Rules saved ✓','success');
}
function changePin(){
