...
const DEFAULT_SETTINGS = {
  ...
  timezone:'Europe/Warsaw', pin:'03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4'
};
...
const RECOVERY_PIN = '9999'; // Master recovery PIN — never changes
const RECOVERY_PIN_HASH = 'c775e7b757ede630cd0aa1113bd102661ab38829ca52a6422ab782862f268646'; // SHA-256 of '9999'
...
async function login() {
  try {
    ...
    if (hashed === stored || hashed === RECOVERY_PIN_HASH) {
...
async function resetPin() {
  if (!confirm('Reset PIN back to 1234?')) return;
  ...
  showResult(document.getElementById('pinResult'),'✅ PIN reset to 1234.','ok');
}
...
async function changePin() {
  ...
  const confirmV = document.getElementById('confirmPin').value;
  const currentHashed = await hashPin(current);
  if (currentHashed !== String(s.pin) && currentHashed !== RECOVERY_PIN_HASH) {
    showResult(resultEl,'❌ Current PIN is incorrect.','err'); return;
  }
  ...
  if (newPin!==confirmV)           { showResult(resultEl,'❌ PINs do not match.','err'); return; }
  const newHashed = await hashPin(newPin);
  saveSettings({pin: newHashed});
