...
function parseTimetableCSV(csv) {
  ...
  if (hi===-1){showBanner('❌ Header row not found. Check sheet tab is correct.','err');return 0;}

  const H=rows[hi].map(c=>(c||'').toLowerCase().trim());
  const col=kw=>H.findIndex(h=>h.includes(kw.toLowerCase()));

  const C = {
    date:      col('exam date '),        // col 8 — note trailing space in header
    room:      col('room'),
    session:   col('session'),
    ...
    code:      col('code'),
    entries:   col('entries'),           // col 6 — Number of Entries (students)
    comments:  col('comments'),
  };

  // entries col — prefer col 6 if "exam date " col found correctly;
  // fallback: find first "entries" col that isn't col 26 (duplicate)
  if (C.entries === -1) {
    C.entries = H.findIndex((h,i) => h === 'entries' && i < 15);
  }
  ...
  for (let i=hi+1;i<rows.length;i++) {
    ...
    // Number of entries (students in this exam)
  ...
  exams=parsed; S.set('exams',exams);
