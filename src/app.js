...
async function emailPDF() {
  ...
  // Choose the dedicated Schedule ID if configured, otherwise fall back to the general one
  ...
  showToast('Generating schedule and sending…', '');
  try {
    const doc = _buildPDF(inv, myExams);
    // Keep local download feature for physical backup records
    doc.save(inv.name.replace(/\s+/g,'_') + '_Invigilation_Schedule.pdf');
    
    // Construct inline HTML rows for a lightweight schedule overview matrix
    const aliases = inv.aliases.map(a => a.toLowerCase().replace(/\/+$/, '').trim());
    const isMain = e => splitNames(e.invigRaw).some(n => {
      const nl = n.toLowerCase().replace(/\/+$/, '').trim();
    ...
    // Send payload matching the exact parameters expected by your new schedule view layout
...
function loadSettingsUI() {
  ...
  setV('ejsScheduleTemplateId', s.ejsScheduleTemplateId);
