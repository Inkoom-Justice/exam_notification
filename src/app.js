...
/** Resolve ALL names in a raw field — returns array, deduped by email */
function resolveAll(rawField) {
  return splitNames(rawField)
    .map(n => resolveInvigilator(n))
    .filter(Boolean)
...
function parseTimetableCSV(csv) {
  ...
  // col 5="Exam Date"(range text, ignored), col 6="Entries"(1st, correct),
  // col 8="Exam Date "(actual datetime, trailing space), col 26="Entries"(duplicate, ignored)
  const C = {
    date:         8,                          // hardcoded — actual exam datetime
...
function _buildPDF(inv, myExams) {
  ...
  doc.text('REGENT INTERNATIONAL SCHOOL', 14, 16);
  ...
  for (let p = 1; p <= total; p++) {
    ...
    doc.text(
      'Regent International School · ' + inv.name + ' · CONFIDENTIAL · Page ' + p + ' of ' + total,
...
async function emailPDF() {
  ...
  const s = getSettings();
  if (!s.ejsPublicKey || !s.ejsServiceId || !s.ejsTemplateId) {
    showToast('Configure EmailJS in Settings first', 'error'); return;
  }
  ...
  try {
    ...
    doc.save(inv.name.replace(/\s+/g,'_') + '_Invigilation_Schedule.pdf');
    // Send email with schedule summary (EmailJS free tier doesn't support attachments)
    // so we send a notification email that the PDF has been generated/shared separately
    await emailjs.send(s.ejsServiceId, s.ejsTemplateId, {
      to_name:        inv.name.split(' ')[0],
      to_email:       resolveEmail(inv),
      exam_subject:   'Your Complete Invigilation Schedule — ' + myExams.length + ' assignments',
      exam_component: 'Please see the attached PDF or the schedule summary below.',
      exam_date:      myExams[0] ? fmtDate(myExams[0].date) + ' to ' + fmtDate(myExams[myExams.length-1].date) : '—',
      exam_time:      '—',
      exam_room:      'See schedule',
      finish_time:    '—',
      ext_finish:     '—',
      num_entries:    '—',
      role:           'Invigilator Schedule',
      readiness_time: '—',
    });
    showToast('PDF downloaded + email sent to ' + resolveEmail(inv) + ' ✓', 'success');
  } catch(e) {
