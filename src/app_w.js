...
function parseTimetableCSV(csv) {
  ...
  // ── Map columns ──────────────────────────────────────────────
  // IMPORTANT: do NOT trim headers before mapping — col 8 is "Exam Date " with
  // a trailing space, which distinguishes it from col 5 "Exam Date" (range text).
  // We hardcode the two ambiguous columns and use keyword search for the rest.
  const H_raw = rows[hi];
  ...
  for (let i = hi + 1; i < rows.length; i++) {
    ...
    else if (/^\d{1,2}\s+[A-Za-z]{3}\s+\d{2,4}$/.test(rawDate)) {
      const months = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',
                      jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};
      const p = rawDate.split(/\s+/);
